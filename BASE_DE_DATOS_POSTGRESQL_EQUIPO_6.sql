...
CREATE TABLE historial_recomendaciones (
    ...
    FOREIGN KEY (id_estudiante) REFERENCES estudiantes(id_estudiante),
    FOREIGN KEY (id_tema) REFERENCES temas_materia(id_tema),
    FOREIGN KEY (id_estrategia) REFERENCES estrategias_ensenanza(id_estrategia)
);
...
CREATE TABLE sesiones_juego (
    ...
    duracion_juego TIME NOT NULL,  -- Cambio: se cambió de INT a TIME para usar una unidad de tiempo más precisa y flexible
    intentos INT NOT NULL,
...
CREATE TABLE item (
    ...
    PRIMARY KEY (id_item),
    FOREIGN KEY (id_tipo_decoracion) REFERENCES tipo_decoracion(id_tipo_decoracion)
);

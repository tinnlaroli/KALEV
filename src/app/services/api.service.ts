...
export class ApiService {
  ...
  // ========== AUTH METHODS ==========
  async login(correo: string, codigoJuego: string): Promise<LoginResponse> {
    try {
      const response = await this.http
        .post<LoginResponse>(`${this.baseUrl}/login-estudiante`, {
          correo,
  ...
  crearMascota(mascotaData: Omit<Mascota, 'id_animal'>): Observable<Mascota> {
    ...
    return from(this.loadAuthToken()).pipe(
      switchMap(() => {
        return this.http
          .post<Mascota>(`${this.baseUrl}/mascotas`, mascotaData, {
  ...
  obtenerMascotasPorJugador(idJugador: number): Observable<Mascota[]> {
    return from(this.loadAuthToken()).pipe(
      switchMap(() => {
        return this.http
          .get<Mascota[]>(`${this.baseUrl}/mascotas/jugador/${idJugador}`, {
            headers: this.getAuthHeaders(),
          })
          .pipe(catchError(this.handleError));
      })
  ...
  // ========== COMPRAS ==========
  realizarCompra(
    compraData: Omit<Compra, 'id_compra' | 'fecha_compra'>
  ): Observable<Compra> {
    ...
    return from(this.loadAuthToken()).pipe(
      switchMap(() => {
        return this.http
          .post<Compra>(`${this.baseUrl}/comprasItems`, compraData, {
            headers: this.getAuthHeaders(),
          })
          .pipe(catchError(this.handleError));
      })
  ...
  obtenerHistorialCompras(idUsuario: number): Observable<Compra[]> {
    return from(this.loadAuthToken()).pipe(
      switchMap(() => {
        return this.http
          .get<Compra[]>(`${this.baseUrl}/comprasItems/usuario/${idUsuario}`, {
            headers: this.getAuthHeaders(),
          })
          .pipe(catchError(this.handleError));
      })
  ...
  private validarDatosCompra(
    data: Omit<Compra, 'id_compra' | 'fecha_compra'>
  ): void {
    if (
      !data.id_usuario ||
      !data.id_item ||
  ...
  async saveSelectedAccessories(accessories: {
    cabeza: any;
    ojos: any;
  }): Promise<void> {
  ...
  registrarMetricasJuego(data: {
    ...
    return this.http.post(
      'https://kalev-apiv2-production.up.railway.app/api/v1/metricas',
      data
    );
  }
}
